...
class Config(BaseModel):
    ...
    main_branch: str = Field(
        default="main", description="Name of the main branch to merge into"
    )
    ...
    always_log: bool = Field(
        default=False, description="Whether to always generate log files"
    )
    ...
    def load(cls, repo_path: Path) -> "Config":
        ...
        try:
            ...
            if "gitsmartcommit" in config_data:
                ...
                if (
                    "log_directory" in config_section
                    and config_section["log_directory"]
                ):
