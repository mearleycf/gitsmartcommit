...
class GitOperationObserver(ABC):
    ...
    @abstractmethod
    async def on_merge_completed(self, success: bool, source_branch: str, target_branch: str) -> None:
...
class ConsoleLogObserver(GitOperationObserver):
    ...
    async def on_merge_completed(self, success: bool, source_branch: str, target_branch: str) -> None:
        if success:
            self.console.print(f"[green]Successfully merged {source_branch} into {target_branch}[/green]")
        else:
            self.console.print(f"[red]Failed to merge {source_branch} into {target_branch}[/red]")
