...
from .prompts import COMMIT_MESSAGE_PROMPT, RELATIONSHIP_PROMPT
        ...
        try:
            ...
            if file_path.is_symlink():
                ...
                if not self._is_safe_path(
                    str(target_path.relative_to(Path(self.repo_path)))
                ):
...
""" + "\n".join(
    ...
    def _fallback_grouping(self, changes: List[FileChange]) -> RelationshipResult:
        ...
        groups = {"main_docs": [], "web_docs": [], "feature_specs": {}, "other": []}
        ...
        with Progress(
            ...
            for unit in commit_units:
                command = CommitCommand(
                    self.repo, unit, self.console, no_verify=self.no_verify
                )
