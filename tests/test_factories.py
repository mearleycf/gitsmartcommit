...
from gitsmartcommit.factories import (
    AgentFactory,
    ClaudeAgentFactory,
    GeminiAgentFactory,
    MockAgentFactory,
...
from gitsmartcommit.models import CommitType, CommitUnit, RelationshipResult
...
def test_gemini_agent_factory():
    ...
    with patch("google.generativeai.configure") as mock_configure:
        test_api_key = os.getenv("TEST_API_KEY", "mock-test-key-for-testing")
        factory = GeminiAgentFactory(model="gemini-pro", api_key=test_api_key)
...
def test_qwen_agent_factory():
    ...
    with patch("os.environ.get", return_value="test-key"), patch(
        "gitsmartcommit.factories.Agent"
    ) as mock_agent_class, patch(
        "gitsmartcommit.factories.ConventionalCommitStrategy"
        ...
        test_api_key = os.getenv("TEST_API_KEY", "mock-test-key-for-testing")
        factory = QwenAgentFactory(model="qwen2.5-coder:7b", api_key=test_api_key)
...
def test_mock_agent_factory():
    ...
    factory = MockAgentFactory(
        mock_relationship_agent=mock_agent, mock_commit_strategy=mock_strategy
    )
