...
from gitsmartcommit.core import CommitType, CommitUnit
...
def mock_analyzer():
    ...
    with patch("gitsmartcommit.cli.ChangeAnalyzer") as mock:
        ...
        analyzer_instance.analyze_changes = AsyncMock(
            return_value=[
                CommitUnit(
...
def test_config_dir_flag_existing_config(cli_runner, tmp_path):
    ...
    with patch("pyperclip.copy") as mock_copy:
        with cli_runner.isolated_filesystem(temp_dir=tmp_path) as td:
            ...
            config_path = Path(td) / ".gitsmartcommit.toml"
            config = Config(
                main_branch="develop", commit_style="simple", remote_name="upstream"
            )
            config.save(Path(td))
...
def test_merge_flag(cli_runner, git_repo, mock_analyzer, mock_committer):
    ...
    result = cli_runner.invoke(
        main, ["--auto-push", "--merge", "--path", str(git_repo)]
    )
...
def test_merge_flag_without_auto_push(
    cli_runner, git_repo, mock_analyzer, mock_committer
):
    """Test the --merge flag without auto-push (should still push and merge)."""
    result = cli_runner.invoke(main, ["--merge", "--path", str(git_repo)])

    assert result.exit_code == 0
    mock_committer.return_value.commit_changes.assert_called_once()
    # Should call push_changes once (for the feature branch) and merge_to_main once
    mock_committer.return_value.push_changes.assert_called_once()
    mock_committer.return_value.merge_to_main.assert_called_once()
...
def test_main_branch_option(cli_runner, git_repo, mock_analyzer, mock_committer):
    ...
    result = cli_runner.invoke(
        main,
        ["--auto-push", "--merge", "--main-branch", "develop", "--path", str(git_repo)],
    )
...
def test_no_verify_option(cli_runner, git_repo, mock_analyzer, mock_committer):
    """Test the --no-verify option."""
    result = cli_runner.invoke(
        main,
        ["--no-verify", "--path", str(git_repo)],
    )

    assert result.exit_code == 0
    # Verify that GitCommitter was called with no_verify=True
    mock_committer.assert_called_once_with(str(git_repo), no_verify=True)
...
def test_commit_style_option(cli_runner, git_repo, mock_analyzer):
    ...
    result = cli_runner.invoke(
        main, ["--commit-style", "conventional", "--path", str(git_repo)]
    )
    assert result.exit_code == 0

    # Test simple style
    result = cli_runner.invoke(
        main, ["--commit-style", "simple", "--path", str(git_repo)]
    )
    assert result.exit_code == 0

    # Test invalid style
    result = cli_runner.invoke(
        main, ["--commit-style", "invalid", "--path", str(git_repo)]
    )
    assert result.exit_code != 0
...
def test_log_file_option(cli_runner, git_repo, mock_analyzer, mock_committer):
    ...
    with patch("gitsmartcommit.cli.FileLogObserver") as mock_observer:
        result = cli_runner.invoke(
            main, ["--log-file", str(log_file), "--path", str(git_repo)]
        )
        ...
        mock_committer.return_value.add_observer.assert_any_call(
...
def test_config_list_default_values(cli_runner, tmp_path):
    ...
    with cli_runner.isolated_filesystem(temp_dir=tmp_path) as td:
        ...
        assert any(
            "main_branch" in line and "main" in line and "default" in line
            for line in settings
        )
        assert any(
            "commit_style" in line and "conventional" in line and "default" in line
            for line in settings
        )
        assert any(
            "remote_name" in line and "origin" in line and "default" in line
            for line in settings
        )
        assert any(
            "auto_push" in line and "False" in line and "default" in line
            for line in settings
        )
        assert any(
            "always_log" in line and "False" in line and "default" in line
            for line in settings
        )
        assert any(
            "log_file" in line and "None" in line and "default" in line
            for line in settings
        )
...
def test_config_list_custom_values(cli_runner, tmp_path):
    ...
    with cli_runner.isolated_filesystem(temp_dir=tmp_path) as td:
        ...
        assert any(
            "main_branch" in line and "develop" in line and "config" in line
            for line in settings
        )
        assert any(
            "commit_style" in line and "simple" in line and "config" in line
            for line in settings
        )
        assert any(
            "remote_name" in line and "upstream" in line and "config" in line
            for line in settings
        )
        assert any(
            "auto_push" in line and "True" in line and "config" in line
            for line in settings
        )
        assert any(
            "always_log" in line and "True" in line and "config" in line
            for line in settings
        )
        assert any(
            "log_file" in line and "custom.log" in line and "config" in line
            for line in settings
        )
...
def test_config_list_with_custom_path(cli_runner, tmp_path):
    ...
    assert any(
        "main_branch" in line and "custom-main" in line and "config" in line
        for line in settings
    )
